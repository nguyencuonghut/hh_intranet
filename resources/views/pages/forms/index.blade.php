    ...
    <div class="row">
        <div class="col-md-4">
            <ul class="nav nav-pills nav-stacked red">
                ...
                <li role="presentation" class="{{ Request::is('employees') ? "active" : "" }}"><a href="{{ route('staffs.index') }}">NHÂN VIÊN</a></li>
            </ul>
        </div>
            ...
            @foreach($posts as $post)
                <div class="media">
                    <div class="media-left media-middle">
                        <a href="{{ route('forms.single', $post->id) }}">
                            <img class="media-object" src="{{ asset('images/' . $post->medium_image) }}">
                        </a>
                    </div>
                    <div class="media-body">
                        <h4 class="media-heading"><a href="{{ route('forms.single', $post->id) }}">{{ $post->title }}</a></h4>
                        {{ mb_substr(strip_tags($post->body), 0, 250) }} {{ strlen(strip_tags($post->body)) > 250 ? '...':'' }}
                        <p><a href="{{ route('forms.single', $post->id) }}" target="_blank">Xem thêm</a></p>
                    </div>
                </div>
            @endforeach
