        ...
        <div class="col-md-8">
            ...
            <div class="col-md-12">
                <div class="panel panel-default panel-red">
                    <div class="panel-heading text-center panel-custom"><b>BIỂU MẪU KHÁC</b></div>
                    <div class="panel-body text-left">
                        @foreach($posts as $m_post)
                            @if($m_post->id != $post->id)
                            <div class="media">
                                <div class="media-left media-middle">
                                    <a href=" {{ route('forms.single', $m_post->id) }}">
                                        <img class="media-object" src="{{ asset('images/' . $m_post->small_image) }}">
                                    </a>
                                </div>
                                <div class="media-body">
                                    <p></p><a href="{{ route('forms.single', $m_post->id) }}">{{ mb_substr($m_post->title, 0, 150) }} {{ strlen($m_post->title) > 150 ? '...' : ''}}</a></p>
                                </div>
                            </div>
                            @endif
                        @endforeach
