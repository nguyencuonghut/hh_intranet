    ...
    <div class="row">
        <div class="col-md-3">
            <ul class="nav nav-pills nav-stacked">
                <li role="presentation" class="active"><a href="{{ route('news.index') }}">TIN TỨC</a></li>
        ...
        <div class="col-md-8">
            ...
            <div class="col-md-12">
                <div class="panel panel-default panel-red">
                    <div class="panel-heading text-center panel-custom"><b>TIN TỨC KHÁC</b></div>
                    <div class="panel-body text-left">
                        @foreach($posts as $m_post)
                            @if($m_post->id != $post->id)
                            <div class="media">
                                <div class="media-left media-middle">
                                    <a href=" {{ route('news.single', $m_post->id) }}">
                                        <img class="media-object" src="{{ asset('images/' . $m_post->small_image) }}">
                                    </a>
                                </div>
                                <div class="media-body">
                                    <p></p><a href="{{ route('news.single', $m_post->id) }}">{{ mb_substr($m_post->title, 0, 150) }} {{ strlen($m_post->title) > 150 ? '...' : ''}}</a></p>
                                </div>
                            </div>
                            @endif
                        @endforeach
