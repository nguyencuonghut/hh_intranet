            ...
            @foreach($relaxs as $relax)
                <div class="media">
                    <div class="media-left media-middle">
                        <a href="{{ route('relaxs.single', $relax->id) }}">
                            <img class="media-object" src="{{ asset('images/' . $relax->medium_image) }}">
                        </a>
                    </div>
                    <div class="media-body">
                        <h4 class="media-heading"><a href="{{ route('relaxs.single', $relax->id) }}">{{ $relax->title }}</a></h4>
                        {{ mb_substr(strip_tags($relax->body), 0, 250) }} {{ strlen(strip_tags($relax->body)) > 250 ? '...':'' }}
                        <p><a href="{{ route('relaxs.single', $relax->id) }}" target="_blank">Xem thêm</a></p>
                    </div>
                </div>
            @endforeach
